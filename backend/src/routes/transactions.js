...
export default async function transactionRoutes(fastify) {
    ...
    async (req, reply) => {
      ...
      if (client_ref) {
        const { rows: existing } = await pool.query(
          "SELECT * FROM transactions WHERE client_ref=$1",
          [client_ref],
        );
      ...
      try {
        ...
        const isNewFormat =
          Array.isArray(paymentsInput) && paymentsInput.length > 0;
        let paymentsData;
        if (isNewFormat) {
          ...
          if (Math.abs(paidTotal - total) > 0.01) {
            throw new Error(
              `Payment total (${paidTotal}) does not match order total (${total})`,
            );
          }
        } else {
          paymentsData = [
        ...
        const txnPaymentMethod =
          paymentsData.length === 1 ? paymentsData[0].method : "mixed";
        const refNo = generateRefNo();

        // Insert transaction with warehouse_id
        const { rows: txnRows } = await client.query(
          `
        INSERT INTO transactions (ref_no, customer_id, cashier_id, subtotal, discount, tax, total, payment_method, warehouse_id, client_ref, created_at)
      ...
      `,
        ...
        for (const item of processedItems) {
          await client.query(
            `
          INSERT INTO transaction_items (transaction_id, product_id, qty, unit_price, discount, subtotal, cost_at_sale, created_at)
        ...
        for (const item of processedItems) {
          ...
          if (stockQty < 0)
            sendOversoldAlert({ ...item.product, stock_qty: stockQty }).catch(
              () => {},
            );
          else if (stockQty <= threshold)
            sendLowStockAlert({ ...item.product, stock_qty: stockQty }).catch(
              () => {},
            );
        }
  ...
  fastify.get(
    ...
    async (req) => {
      ...
      if (cashier_id) {
        where += ` AND t.cashier_id=$${pIdx++}`;
        params.push(cashier_id);
      }
      if (status) {
        where += ` AND t.status=$${pIdx++}`;
        params.push(status);
      }
      if (search) {
        where += ` AND (t.ref_no ILIKE $${pIdx++} OR u.name ILIKE $${pIdx++})`;
  ...
  fastify.post(
    ...
    async (req, reply) => {
      ...
      if (!rows[0])
        return reply
          .code(404)
          .send({ error: "Transaction not found or cannot be voided" });
      ...
      await pool.query("UPDATE transactions SET status='voided' WHERE id=$1", [
        id,
      ]);
