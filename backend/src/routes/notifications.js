...
export default async function notificationRoutes(fastify) {
  ...
  fastify.post('/api/notifications/test-ai-summary', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    ...
    const { rows } = await pool.query(
      "SELECT key, value FROM settings WHERE key IN ('telegram_bot_token','telegram_chat_id','store_name','claude_api_key','ai_summary_enabled')"
    )
    const s = Object.fromEntries(rows.map(r => [r.key, r.value]))

    if (!s.claude_api_key) {
      return reply.code(400).send({ error: 'Claude API key not configured' })
    }
