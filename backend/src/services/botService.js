    ...
    switch (cmd) {
      ...
      case '/stock': {
        const { rows: oversold } = await pool.query(
          "SELECT name, stock_qty FROM products WHERE stock_qty < 0 AND is_active=true ORDER BY stock_qty ASC LIMIT 10"
        )
        const { rows: low } = await pool.query(
          "SELECT name, stock_qty FROM products WHERE stock_qty >= 0 AND stock_qty <= 5 AND is_active=true ORDER BY stock_qty ASC LIMIT 10"
        )
      ...
      case '/top': {
        ...
        `, [today])
        reply = `🏆 <b>Top Products Today</b>\n\n` +
          (rows.length ? rows.map((r, i) => `${i + 1}. ${r.name} (${r.qty} sold)`).join('\n') : 'No sales today')
        break
      }
      case '/cashiers': {
        const { rows } = await pool.query(`
        ...
        `, [today])
        reply = `👤 <b>Cashiers Today</b>\n\n` +
          (rows.length ? rows.map(r => `${r.name}: ${r.count} txns · ₱${parseFloat(r.total).toFixed(2)}`).join('\n') : 'No activity')
        break
      }
      case '/refunds': {
        const { rows } = await pool.query(`
        ...
        `, [today])
        reply = `↩️ <b>Refunds Today</b>\n\n` +
          (rows.length ? rows.map(r => `${r.ref_no}: ₱${parseFloat(r.total_refund_amount).toFixed(2)} — ${r.reason}`).join('\n') : 'No refunds today')
        break
      }
      case '/txn': {
        const ref = args[0]
        if (!ref) { reply = 'Usage: /txn TXN-xxxx'; break }
        const { rows } = await pool.query(
          'SELECT t.*, u.name as cashier_name FROM transactions t LEFT JOIN users u ON u.id=t.cashier_id WHERE t.ref_no=$1',
          [ref]
        )
        if (!rows[0]) { reply = `Transaction ${ref} not found`; break }
        const t = rows[0]
        ...
        reply = `🧾 <b>${t.ref_no}</b>\n` +
          ...
          `<b>Items:</b>\n` +
          items.map(i => `• ${i.name} ×${i.qty} = ₱${parseFloat(i.subtotal).toFixed(2)}`).join('\n')
        break
      }
      case '/status': {
...
export async function startBot() {
  ...
  while (polling) {
    try {
      ...
      if (!ok) { await sleep(5000); continue }
