...
export async function pushToCloud() {
  ...
  if (!cloudUrl)
    return { pushed: 0, skipped: 0, message: "CLOUD_API_URL not set" };
  ...
  if (!pending.length)
    return { pushed: 0, skipped: 0, message: "Nothing to sync" };
  ...
  console.log(JSON.stringify(transactions, null, 2)); // Debug log

  if (!transactions.length)
    return { pushed: 0, skipped: 0, message: "No valid transactions found" };
